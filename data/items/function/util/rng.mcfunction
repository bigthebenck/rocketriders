...
execute if predicate game:item_pool/lunging_spear if entity @e[x=0,type=armor_stand,tag=Selection,limit=1,tag=!givenLungingSpear] run summon marker 0 0 0 {Tags:["rngLungingSpear","rng2"]}
...
execute unless predicate game:game_rules/item_stacking/on as @e[x=0,type=marker,tag=rngSelected,tag=rngLungingSpear] as @e[x=0,type=item] if items entity @s contents *[custom_data~{id:"lunging_spear"}] run function items:deduct
...
execute as @e[x=0,type=marker,tag=rngSelected,tag=rngLungingSpear] as @a[x=0,tag=getItem,predicate=custom:team/any_playing_team] run function items:util/givelungingspear
